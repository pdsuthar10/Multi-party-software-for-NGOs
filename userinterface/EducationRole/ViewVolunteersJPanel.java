...
public class ViewVolunteersJPanel extends javax.swing.JPanel {
    ...
    private void populateVolunteerDetails(UserAccount userAccount){
            ...
            skillsjTextArea.setText(userAccount.getVolunteer().getAddSkills());
            ...
            if(userAccount.getVolunteer().getExpertise().contains("MS Word")){
                wordjCheckBox.setSelected(true);
            }
            if(userAccount.getVolunteer().getExpertise().contains("MS Excel")){
                exceljCheckBox.setSelected(true);
            }
            if(userAccount.getVolunteer().getExpertise().contains("PowerPoint")){
                pptjCheckBox.setSelected(true);
            }
            if(userAccount.getVolunteer().getExpertise().contains("Web Browsing")){
                webjCheckBox.setSelected(true);
            }
            if(userAccount.getVolunteer().getExpertise().contains("Email Checking")){
     ...
     private void populateLang(UserAccount userAccount){
        ...
        for(String lang: userAccount.getVolunteer().getLanguages()){
    ...
    private void populateEducation(UserAccount userAccount){
        ...
        educationjTextArea.setText("");
        int count = 1;
        for(Map.Entry<String,String> entry : userAccount.getVolunteer().getEducationList().entrySet()){
    ...
    private void cancelRequestJButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cancelRequestJButtonActionPerformed
        ...
        if(selectedRow>=0){
            ...
            int selectionResult = JOptionPane.showConfirmDialog(null, "Are you sure to Cancel this volunteer request??","Warning",selectionButton);
